...
| **M6 — Agent surface** | 13–14 | MCP server (stdio), full tool set per SPEC §5.1, attribution in ledger | a Claude agent runs the loop end-to-end — including resuming a day-old workspace — and a human replays the ledger |
...
- **Eval week:** run the full SPEC §6 suite. Gates (matching SPEC §6 exactly):
  quote fidelity 100% canonical-match on `verified-*` pins; citation-support
  rate ≥95% on the annotation set pre-built from M3–M5 dogfooding pins;
  frontier proxy gate — frontier ordering beats relevance-only on recall@budget
  on the majority of benchmarks, else frontier ships marked *experimental* and
  the release notes say so (kill criterion honored, no silent shipping of a
  broken headline feature). Verification-tier shares and the agent-integration
